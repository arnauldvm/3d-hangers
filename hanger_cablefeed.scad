...
hanger_big_depth_mm = cablefeed_width_mm/2;
...
minkowski() {
    ...
    difference() {
        ...
        union() {
            ...
            difference() {
                pie_slice(r=hanger_small_depth_mm, angle=60);
                pie_slice(r=hanger_small_depth_mm-kernel_fine_thickness_mm, angle=60);
            }

            translate([kernel_cablefeed_width_mm/2-hanger_big_depth_mm, kernel_thickness_mm])
            difference() {
                pie_slice(r=hanger_big_depth_mm, angle=140);
                pie_slice(r=hanger_big_depth_mm-kernel_fine_thickness_mm, angle=140);
            }
        }
        rotate([0,90,90])
